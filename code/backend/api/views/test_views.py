...
class TestViewSet(viewsets.ModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        """
        Custom create method using the serializer.
        """
        serializer = TestCreationSerializer(data=request.data, context={'request': request})

        if serializer.is_valid():
            test = serializer.save()
            self.evaluate_llm(test)
            test.completed_at = timezone.now()

            self.compute_test_metrics(test)
    ...
    def evaluate_llm(self, test):
        ...
        for question in questions:
            answer, explanation = self.query_llm(llm_model, question)
    ...
    def query_llm(self, llm_model, question):
        """
